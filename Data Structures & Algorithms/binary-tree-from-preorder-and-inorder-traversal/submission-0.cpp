...
public:
    ...
    TreeNode* build(vector<int> &preorder, int preStart, int preEnd, vector<int> &inorder,int inStart, int inEnd, map<int,int> inMap){
        ...
        root->left = build(preorder,preStart+1,preStart+leftNums,inorder,inStart,inRoot-1,inMap);
        root->right = build(preorder,preStart+leftNums+1,preEnd,inorder,inRoot+1,inEnd,inMap);
        return root;
    }
};
